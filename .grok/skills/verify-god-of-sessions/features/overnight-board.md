...
- `overnight-setup` shows `Finish Overnight setup` when no ready CLI worker exists.
...
- **Empty or setup.** Read the primary region `Overnights`. If no CLI is installed, heading `Finish Overnight setup` and button `Open Settings`. If CLIs can run and tonight is empty, heading `No Overnight is ready tonight`. Both are pass states, not bugs.
