...
Morrow is the first screen after GitHub identity. Above the chat, Morrow shows up to three tonight cards. Every card starts checked. Unchecking leaves that Overnight out. The one start button runs only the remaining checks. Each card names the worker.
...
- `worker-label` shows `providerLabel` on every card and does not show `providerReason` on the card.
...
This feature requires **live Electron verification** (`gos-verify.mjs`). A synthetic-only pass from `drive.mjs` is incomplete because it injects fake IPC handlers that return fixture data with 3 cards. The real window may show heading `Tonight's 3 cards` and a Settings connect control when no model is connected.
...
- **See tonight cards.** The region `Tonight's overnights` is visible with up to three cards. If no cards appear and the screen shows heading `Tonight's 3 cards` with connect in Settings, the live drive is **RED**: the precondition (connected model) is unmet.
...
- **See three checked cards.** Three checkboxes exist, all checked. Outcomes `Ship the login fix`, `Backfill coverage`, and `Tighten the release checklist` are visible. Each line includes a worker name (`Claude Code`, `Codex`, `Grok Build`). Selection rationale must not appear on the card.
...
- Missing worker name is a fail; selection rationale on the card is a fail.
...
- **Synthetic-only green is a rotten map.** The real window may show empty tonight slots and a Settings connect path instead of cards when no model is connected. Always require live drive evidence.
