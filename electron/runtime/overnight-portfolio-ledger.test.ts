...
describe("Overnight portfolio durable ledger", () => {
  ...
  it("consumes one exact path-free V3 item launch and never persists its private paths", async () => {
    const { dataDir, authority } = await setup();
    const item = authority.plan.items[0];
    const frozen = authority.items[0];
    const plan = new OvernightPortfolioCoordinator({ now: () => new Date("2026-08-26T18:00:00.000Z") })
      .prepare([item], { "provider:codex": 1 }, { planId: authority.plan.id });
    const fixedRoot = "/private/worktrees/one";
    const worktreeKey = "/private/worktrees/one";
    const runtimeDirectory = "/private/runtime/one";
    const containmentAuthority = {
      version: 3 as const,
    ...
    const v3Authority: OvernightPortfolioExecutionAuthority = {
      ...
      items: [{ itemId: frozen.itemId, brief: frozen.brief, containmentAuthority }],
    };
    const ledger = new OvernightPortfolioLedger({ dataDir });
    await ledger.saveAuthority(v3Authority);
    await ledger.claimAuthority(v3Authority.plan.id, "run_v3", "2026-08-26T18:01:00.000Z");
    await ledger.createRun({
      id: "run_v3",
      planId: v3Authority.plan.id,
      title: "V3 run",
      startedAt: "2026-08-26T18:01:00.000Z",
      deadlineAt: "2026-08-26T19:01:00.000Z",
      items: [{ itemId: item.id, provider: item.provider }],
    });
    await ledger.writeItemState("run_v3", {
      itemId: item.id,
      provider: item.provider,
      providerLabel: "Codex",
      status: "running",
    ...
    const approvalClaimSha256 = overnightApprovedLaunchClaimSha256({
      planId: v3Authority.plan.id,
      approvalFingerprint: v3Authority.plan.approvalFingerprint,
      runId: "run_v3",
      itemId: item.id,
    ...
    const concurrent = await Promise.all([
      ledger.consumeApprovedLaunchClaim(input),
      new OvernightPortfolioLedger({ dataDir }).consumeApprovedLaunchClaim(input),
    ]);
    expect(concurrent.filter(Boolean)).toHaveLength(1);
    await expect(ledger.consumeApprovedLaunchClaim({ ...input, worktreeKey: "/private/forged" }))
    ...
    const stored = await readFile(join(
      ...
      "run_v3",
      "launch-preparations",
      `${item.id}.claimed.json`,
    ), "utf8");
    expect(stored).not.toContain(fixedRoot);
    expect(stored).not.toContain(runtimeDirectory);
    expect(stored).toContain(containmentAuthority.executionRootSha256);
  });
