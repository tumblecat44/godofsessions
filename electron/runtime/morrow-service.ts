...
The returned Overnight set may contain runnable, clarify, and no-run purposes across exactly four execution routes: Claude Code, Codex, Grok Build, and Pi Agent. Provider readiness and containment evidence, not the source session's provider, determine whether a route can run. Cursor, Hermes, and OpenClaw session history may remain as read-only evidence, but they are never execution routes for new work. Preserve every independent candidate. Include every runnable purpose that fits the proven 450-minute schedule; when the complete runnable set does not fit, keep every candidate in an editable selection instead of choosing an arbitrary item count or silently discarding work. A date with no runnable Overnight is valid.
