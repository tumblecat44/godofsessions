...
async function persistedProviderRecoveryFixture(mode: "live" | "missing" | "mismatch") {
  ...
  if (mode === "live") {
    ...
    livePids = await waitForJsonFile(markerPath) as { providerPid: number; grandchildPid: number };
...
describe("Morrow service dogfood", () => {
  it("stays conversational, reads quietly, and gates writes through Pi tool events", async () => {
    ...
    expect(observedSystemPrompt).toContain("makespan exceeds 450 minutes must remain editable");
    expect(observedSystemPrompt).toContain("direct the user to Orchestrate");
