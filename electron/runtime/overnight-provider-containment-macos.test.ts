...
describe("macOS Overnight provider containment host", () => {
  ...
  it("observes static identity with system codesign only and never spawns the provider executable", async () => {
    const native = await nativeFixture("codex-static-native", "static-identity");
    const harness = host({
      scripts: [
        { exitCode: 0 },
    ...
    await expect(harness.value.inspectExecutableStatic?.(native.path)).resolves.toEqual({
      realpath: native.path,
      sha256: createHash("sha256").update(native.contents).digest("hex"),
      invocationIdentitySha256: expect.stringMatching(/^[a-f0-9]{64}$/u),
      signatureValid: true,
    ...
    expect(harness.commands.calls.some(({ executable }) => executable === native.path)).toBe(false);
  });

  it("keeps dynamic attestation identity exactly aligned with the static planning observation", async () => {
    const native = await nativeFixture("codex-shared-identity-native", "shared-static-dynamic-identity");
    const staticHarness = host({
      scripts: [
        { exitCode: 0 },
        { exitCode: 0, stderr: `TeamIdentifier=${OPENAI_CODEX_TEAM_IDENTIFIER}\n` },
      ],
    });
    const dynamicHarness = host({
      scripts: [
        { exitCode: 0 },
        { exitCode: 0, stderr: `TeamIdentifier=${OPENAI_CODEX_TEAM_IDENTIFIER}\n` },
        { exitCode: 0, stdout: "codex-cli 1.2.3" },
    ...
    const staticObservation = await staticHarness.value.inspectExecutableStatic!(native.path);
    const dynamicObservation = await dynamicHarness.value.inspectExecutable(native.path, ["--version"]);
    const { version, ...dynamicIdentity } = dynamicObservation;

    expect(dynamicIdentity).toEqual(staticObservation);
    expect(version).toBe("codex-cli 1.2.3");
    expect(staticHarness.commands.calls.some(({ executable }) => executable === native.path)).toBe(false);
    expect(dynamicHarness.commands.calls.filter(({ executable }) => executable === native.path)).toHaveLength(1);
  });
  ...
  it("fails closed when any wrapper invocation-identity file drifts after resolution", async () => {
    ...
    const error = await harness.value.inspectExecutableStatic!(native.path).catch((value: unknown) => value);
  ...
  it("fails closed when the resolver changes a wrapper's native target", async () => {
    ...
    await expect(harness.value.inspectExecutableStatic!(first.path))
