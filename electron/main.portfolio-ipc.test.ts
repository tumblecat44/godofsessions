...
describe("portfolio IPC boundary", () => {
  it("accepts only one of the four execution provider IDs for explicit verification", async () => {
  ...
  it("rejects former read-only history providers for new plans", async () => {
    ...
    for (const provider of ["cursor", "hermes", "openclaw"]) {
      await expect(invoke(provider)).rejects.toThrow(/provider/);
    }
  });
