...
const bridge: MorrowBridge & { openExternal(url: string): Promise<void> } = {
  ...
  refreshOvernightProviders: () => ipcRenderer.invoke("morrow:refresh-overnight-providers"),
  ...
  listOvernightBoardTickets: (overnightId) => ipcRenderer.invoke("morrow:list-overnight-board-tickets", overnightId),
  moveOvernightBoardTicket: (input) => ipcRenderer.invoke("morrow:move-overnight-board-ticket", input),
  addOvernightBoardTicket: (input) => ipcRenderer.invoke("morrow:add-overnight-board-ticket", input),
  ensureOvernightBoardTickets: (input) => ipcRenderer.invoke("morrow:ensure-overnight-board-tickets", input),
  scheduleOvernightNight: (request) => ipcRenderer.invoke("morrow:schedule-overnight-night", request),
  cancelOvernightNight: (cardId) => ipcRenderer.invoke("morrow:cancel-overnight-night", cardId),
  overnightBranchLog: (cardId) => ipcRenderer.invoke("morrow:overnight-branch-log", cardId),
  openExternal: (url) => ipcRenderer.invoke("morrow:open-external", url),
  revealRoot: () => ipcRenderer.invoke("morrow:reveal-root"),
  revealOvernightStore: () => ipcRenderer.invoke("morrow:reveal-overnight-store"),
