...
describe("Overnight awareness outside Orchestrate", () => {
  it("does not show attention when a newly received run has a fresh newer heartbeat", async () => {
    ...
    resolveBootstrap({
      rootName: "morrow-root", onboardingComplete: true, providers: [], models: [], conversations: [], thinkingLevel: "medium", language: "en",
      orchestration: {
  ...
  it("keeps a low-frequency background watch and clears the running badge at completion", async () => {
    ...
    expect(screen.getByRole("button", { name: "Overnight" })).not.toHaveTextContent("RUNNING");
  });
  ...
  it("resumes polling after a failed user refresh invalidates an older response generation", async () => {
    ...
    render(<App />);
    await act(async () => { await Promise.resolve(); });
    fireEvent.click(screen.getByRole("button", { name: "Overnight · active" }));
  ...
  it("keeps a failed Stop request visible instead of dropping the rejection", async () => {
    ...
    const { default: App } = await import("./App");
    render(<App />);
    await waitFor(() => expect(screen.getByRole("button", { name: "Overnight · active" })).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "Overnight · active" }));
...
describe("portfolio awareness outside Orchestrate", () => {
  it("polls a portfolio-only run and shows the active item count until Morning Review", async () => {
    ...
    expect(screen.getByRole("button", { name: "Overnight · 2 Overnights active" })).toHaveTextContent("2 ACTIVE");
    await act(async () => { await vi.advanceTimersByTimeAsync(10_000); });
    expect(bridge.overnightSnapshot).toHaveBeenCalledOnce();
    expect(screen.getByRole("button", { name: "Overnight" })).not.toHaveTextContent("ACTIVE");
    fireEvent.click(screen.getByRole("button", { name: "Overnight" }));
    expect(screen.getByRole("article", { name: "Portfolio morning review" })).toHaveTextContent("Copy review failed.");
  });
  ...
  it("uses the portfolio stop boundary instead of the legacy worker stop", async () => {
    ...
    const { default: App } = await import("./App");
    render(<App />);
    await waitFor(() => expect(screen.getByRole("button", { name: "Overnight · 1 Overnight active" })).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "Overnight · 1 Overnight active" }));
    fireEvent.click(await screen.findByRole("button", { name: "Stop all for this night" }));
  ...
  it("moves a focused Night Plan outcome into the Morrow composer without exposing execution details", async () => {
    const item = {
      id: "ui-result", stableKey: "ui-result", origin: "continuation" as const, title: "Repair the settings flow",
      outcome: "Settings stay connected after restart", verification: "npm test -- settings", provider: "codex" as const,
      providerLabel: "Codex", providerReason: "Codex is ready.", estimatedMinutes: 90, startMinute: 0, endMinute: 90,
      isolation: "isolated" as const, dependencyIds: [], conflictKeys: [], writeScopes: ["src"], risks: [], selectedSessions: [],
      commandPreview: "PRIVATE EXECUTION DETAILS",
    };
    const plan = {
      id: "plan-discuss", status: "draft" as const, title: "Tonight's settings result", items: [item], totalMinutes: 90,
      peakParallelism: 1, approvalFingerprint: "sha256:discussion", createdAt: "2026-08-26T07:00:00.000Z", expiresAt: "2099-08-26T07:05:00.000Z",
    };
    const state: BootstrapState = {
      rootName: "morrow-root", onboardingComplete: true,
      providers: [{ id: "test", name: "Test", connected: true, authTypes: ["api_key"] }],
      models: [{ id: "model", provider: "test", name: "Test model", reasoning: false }], selectedModel: { provider: "test", id: "model" },
      conversations: [], thinkingLevel: "medium", language: "en",
      orchestration: {
        context: { date: "2026-08-26", timeZone: "UTC", generatedAt: "2026-08-26T07:00:00.000Z", totalSessions: 0, providerCounts: {}, sessions: [], warnings: [], methodology: "test" },
        plans: [], runs: [], providerRoutes: [{ provider: "codex", label: "Codex", status: "ready" }],
        portfolioPlans: [plan], portfolioRuns: [],
        portfolioAssessments: [{
          id: "assessment-discuss", requestKind: "discover", disposition: "recommend", planId: plan.id, selectionId: plan.id,
          createdAt: "2026-08-26T07:00:00.000Z", contextGeneratedAt: "2026-08-26T07:00:00.000Z",
          candidates: [{
            stableKey: item.stableKey, origin: item.origin, disposition: "recommend", title: item.title,
            rationale: "This is a bounded result.", reasonCodes: ["unfinished_work", "bounded_scope", "clear_verification", "overnight_leverage"],
            selectedSessions: [], excludedSessions: [], outcome: item.outcome, verification: item.verification, preferredProvider: "codex",
            providerReason: item.providerReason, estimatedMinutes: item.estimatedMinutes, risks: [], questions: [], dependencyKeys: [], conflictKeys: [], writeScopes: ["src"],
          }],
        }],
      },
    };
    const bridge: MorrowBridge = {
      bootstrap: vi.fn(async () => state), startConversation: vi.fn(), openConversation: vi.fn(), sendMessage: vi.fn(), abort: vi.fn(),
      setModel: vi.fn(), setThinkingLevel: vi.fn(), answerApproval: vi.fn(), connectProvider: vi.fn(), answerAuthPrompt: vi.fn(),
      disconnectProvider: vi.fn(), finishOnboarding: vi.fn(), refreshDailyContext: vi.fn(), replanOvernightPortfolio: vi.fn(),
      startOvernightPortfolio: vi.fn(), stopOvernightPortfolio: vi.fn(), startOvernight: vi.fn(), stopOvernight: vi.fn(), openExternal: vi.fn(),
      onEvent: () => () => undefined,
    };
    window.morrow = bridge;
    const { default: App } = await import("./App");
    ...
    fireEvent.click(screen.getByRole("button", { name: `Open details for ${item.outcome}` }));
    fireEvent.click(screen.getByRole("button", { name: "Revise this outcome with Morrow" }));
...
describe("Morrow provider onboarding", () => {
  it("shows and clears provider authentication surfaces during onboarding", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: /Sign in with your OpenAI Codex account/ }));
  ...
  it("keeps a newer conversation bootstrap ahead of a delayed initial view transition", async () => {
    ...
    try {
      ...
      expect(screen.getByTitle("Fixed execution root")).toHaveTextContent("latest-root");
  ...
  it("suspends every visible plan authority while daily context refresh is unresolved or failed", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("button", { name: "Review & run in Overnight" })).toBeEnabled();

    fireEvent.click(screen.getByRole("button", { name: "Overnight" }));
    fireEvent.click(screen.getByRole("button", { name: "Refresh today" }));
    expect(bridge.refreshDailyContext).toHaveBeenCalledTimes(1);
    fireEvent.click(screen.getByRole("button", { name: "Ask Morrow" }));
    const inlineRun = await screen.findByRole("button", { name: "Review & run in Overnight" });
    ...
    expect(screen.getByRole("button", { name: "Review & run in Overnight" })).toBeDisabled();
  ...
  it("lets a newly prepared plan establish authority after a failed daily context refresh", async () => {
    ...
    expect(screen.getByRole("button", { name: "Prepare as a portfolio" })).toBeEnabled();
    expect(bridge.startOvernight).not.toHaveBeenCalled();
  });
