...
function PortfolioOrchestrateView(props: OrchestrateViewProps) {
  ...
  const morningRun = !activeRun
    ? portfolioRuns.find((run) => run.id === selectedReviewRunId && terminalPortfolioStatuses.has(run.status)) ?? defaultMorningRun
    : undefined;
    ...
    <main className="orchestrate-view h-dvh overflow-y-auto bg-night px-[clamp(32px,5vw,80px)] pb-16 pt-[clamp(58px,7vh,82px)] text-ink max-[1120px]:px-9" hidden={props.hidden}>
      ...
      {pastRuns.length > 0 && (
        <Surface className="orchestrate-section portfolio-past-runs">
          <div className="orchestrate-section__title"><Activity size={17} /><div><span>{ko ? "이전 밤" : "PAST NIGHTS"}</span><h2>{ko ? "지난 포트폴리오" : "Earlier portfolios"}</h2></div></div>
          <div className="portfolio-run-list">{pastRuns.map((run) => <PortfolioRunRow key={run.id} run={run} ko={ko} onOpen={() => setSelectedReviewRunId(run.id)} />)}</div>
        </Surface>
      )}
    </main>
  );
...
function PortfolioActiveRun({ run, plan, ko, onStop }: { run: OvernightPortfolioRunSummary; plan?: OvernightPortfolioPlanSummary; ko: boolean; onStop(id: string): Promise<void> }) {
  ...
  useEffect(() => setConfirmingStop(false), [run.id, run.status]);
  return <article className="portfolio-active-run" aria-label={ko ? "실행 중인 포트폴리오" : "Portfolio in progress"}><header><div><span>{ko ? "밤사이 결과 준비 중" : "PREPARING YOUR OUTCOMES"}</span><h2>{ko ? `${workingCount}개 작업 중 · ${waitingCount}개 대기 중` : `${workingCount} working · ${waitingCount} waiting`}</h2><p>{ko ? "추측한 진행률 대신 각 결과의 실제 작업 상태를 보여줍니다." : "Each result shows its actual worker state instead of a guessed completion percentage."}</p></div><em>{portfolioRunStatusLabel(run.status, ko)}</em></header><div className="portfolio-running-items">{run.items.map((item, index) => <PortfolioRunItem key={item.itemId} item={item} planItem={plan?.items.find((entry) => entry.id === item.itemId)} index={index} ko={ko} />)}</div><footer>{confirmingStop ? <div className="mr-auto grid gap-2 text-[11px] text-ink-muted" role="alert"><p>{ko ? "실행 중인 작업을 중지하고 대기 중인 작업은 시작하지 않습니다. 이미 도착한 보고는 보존됩니다." : "Running work will stop and waiting work will not start. Reports already received will be kept."}</p><span className="flex gap-2"><button type="button" onClick={() => setConfirmingStop(false)}>{ko ? "계속 실행" : "Keep running"}</button><button type="button" className="is-danger" onClick={() => void onStop(run.id)}><CircleStop size={15} />{ko ? "전체 실행 중지 확인" : "Confirm stop all"}</button></span></div> : <><p><ShieldCheck size={14} />{ko ? "한 결과가 실패해도 독립적인 결과는 계속 준비할 수 있습니다." : "Independent results can continue when one item fails."}</p><button type="button" disabled={run.status === "stopping"} onClick={() => setConfirmingStop(true)}><CircleStop size={15} />{run.status === "stopping" ? (ko ? "중지하는 중…" : "Stopping…") : (ko ? "전체 Overnight 중지" : "Stop Overnight")}</button></>}</footer></article>;
}
...
function PortfolioRunRow({ run, ko, onOpen }: { run: OvernightPortfolioRunSummary; ko: boolean; onOpen(): void }) {
  const reportCount = run.items.filter((item) => Boolean(item.result?.report)).length;
  return <article><button type="button" onClick={onOpen} aria-label={ko ? `${run.title} 아침 검토 다시 열기` : `Reopen morning review for ${run.title}`}><header><div><span>{formatAbsoluteDateTime(run.startedAt, ko)}</span><h3>{run.title}</h3></div><em>{portfolioRunStatusLabel(run.status, ko)}</em></header><p>{ko ? `${run.items.length}개 작업 · 작업자 보고 ${reportCount}개` : `${run.items.length} items · ${reportCount} worker reports`}</p><small>{ko ? "전체 검토 다시 열기" : "Reopen full review"}<ChevronRight size={13} /></small></button></article>;
}
