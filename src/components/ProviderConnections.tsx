      ...
      {visible.map((provider) => (
        <Surface className={cn("provider-card grid min-h-[76px] grid-cols-[40px_minmax(0,1fr)_auto] items-center gap-3 rounded-[14px] bg-white/[0.018] px-3 py-2.5 shadow-none", provider.connected && "is-connected border-teal/20 bg-teal/[0.025]")} key={provider.id}>
          ...
          <div className="provider-card__actions flex items-center gap-1.5">
            {provider.connected ? compact ? <span className="provider-ready font-mono text-[9px] tracking-[0.12em] text-teal">{ko ? "연결됨" : "CONNECTED"}</span> : <Button size="sm" onClick={() => void onDisconnect(provider.id)}><LogOut size={13} />{ko ? "연결 해제" : "Disconnect"}</Button> : provider.authTypes.map((authType) => <Button variant={authType === "oauth" ? "primary" : "secondary"} size="sm" className={authType === "oauth" ? "primary" : ""} key={authType} onClick={() => void onConnect(provider.id, authType)}>{authType === "oauth" ? <Link2 size={13} /> : <KeyRound size={13} />}{authType === "oauth" ? (ko ? "로그인" : "Sign in") : "API key"}</Button>)}
          </div>
        </Surface>
      ))}
      {compact && providers.length > compactProviderLimit && <Button variant="ghost" className="provider-more col-span-full w-full" aria-expanded={expanded} onClick={() => transitionState(() => setExpanded((value) => !value))}>{expanded ? (ko ? "간단히 보기" : "Show fewer services") : (ko ? `다른 AI 서비스 ${hiddenCount}개 보기` : `Show ${hiddenCount} more services`)}<span className={`state-icon-swap ${expanded ? "is-active" : ""}`} aria-hidden="true"><span className="state-icon-swap__active"><ChevronUp size={14} /></span><span className="state-icon-swap__inactive"><ChevronDown size={14} /></span></span></Button>}
    </div>
  );
}
