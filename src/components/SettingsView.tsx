...
export function SettingsView({ state, error, onConnect, onDisconnect, onLanguage, onReplayOnboarding }: {
  ...
  return (
    <main className="settings-view morrow-settings">
      ...
      <section className="settings-section"><div className="settings-section__intro"><h2>Model providers</h2><p>Credentials are managed by the embedded Pi runtime. Morrow never asks a separate Pi process to do this.</p></div>{error && <div className="settings-error" role="alert">Morrow couldn’t complete that connection. <small>{error}</small></div>}<ProviderConnections state={state} onConnect={onConnect} onDisconnect={onDisconnect} /></section>
