...
describe("Orchestrate Overnight state machine", () => {
  ...
  it("renders no_run as a trustworthy answer with no approval affordance", () => {
    ...
    expect(screen.getByText("No session was selected as evidence, and no worker was created.")).toBeInTheDocument();
    expect(screen.queryByText(/worker receives only the approved outcome/i)).not.toBeInTheDocument();
  ...
  it("keeps decision evidence visible during refresh and prevents a competing refresh while assessing", () => {
    ...
    expect(screen.getByRole("button", { name: "Refresh today" })).toBeDisabled();
  ...
  it("keeps an earlier draft read-only and prepares its outcome as a current portfolio", async () => {
    ...
    expect(card).toHaveTextContent("Outcome still needed");
    ...
    expect(card).toHaveTextContent("Useful unattended");
    ...
    fireEvent.click(screen.getByRole("button", { name: "Prepare as a portfolio" }));
  ...
  it("uses clear Korean worker language and keeps unused sessions in one plan-time drawer", () => {
    ...
    expect(card).toHaveTextContent("포트폴리오 방식으로 다시 준비");
    expect(screen.getByRole("button", { name: "포트폴리오로 다시 준비" })).toBeEnabled();
    expect(screen.getByRole("heading", { name: "계획 준비 시점의 로컬 AI 세션 2개" })).toBeInTheDocument();
    const excluded = screen.getByText("이번 Overnight에서 사용하지 않는 세션 보기 (1) · 계획 준비 시점 기준");
  ...
  it("shows every unused session in one separate collapsed area with notable reasons", () => {
    ...
    const summary = screen.getByText("View sessions not used for this Overnight (1) · when prepared");
  ...
  it("does not mix today's sessions into an earlier run whose full context was never retained", () => {
    ...
    expect(screen.getByText("Earlier run · full context not retained")).toBeInTheDocument();
    ...
    expect(screen.getByText(/unused sessions cannot be reconstructed/i)).toBeInTheDocument();
    expect(screen.queryByText(/View sessions not used for this Overnight/)).not.toBeInTheDocument();
  });
  ...
  it("keeps an earlier plan visible while refresh blocks only its portfolio re-preparation", async () => {
    ...
    expect(screen.getByRole("article", { name: "Earlier-version Overnight plan" })).toHaveTextContent(plan.outcome);
    const retryButton = screen.getByRole("button", { name: "Prepare as a portfolio" });
    ...
    const enabledPrepareButton = screen.getByRole("button", { name: "Prepare as a portfolio" });
  ...
  it("shows one real worker with a seven-hour clock, liveness, activity, and excluded sessions outside the workflow", () => {
    ...
    expect(worker).toHaveTextContent(/automatic stop at Aug 20/);
    expect(within(worker).getByRole("progressbar", { name: "Approved run time used" })).toHaveValue(30 * 60 * 1_000);

    const excluded = screen.getByText("View sessions not used for this Overnight (1) · when prepared");
    expect(excluded.closest("details")).not.toHaveAttribute("open");
  ...
  it("keeps the plan-time context date and provider counts after the local day changes", () => {
    ...
    expect(frozenDeck.getByRole("heading", { name: "2 local AI sessions when prepared" })).toBeInTheDocument();
    ...
    expect(frozenDeck.getByText("1 context note · when prepared")).toBeInTheDocument();
  ...
  it("keeps the current morning review visible while daily context refreshes", () => {
    ...
    expect(screen.getByRole("heading", { name: "0 context sessions retained from the earlier run" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Refresh today" })).toBeDisabled();
  });
  ...
  it("removes Run and returns to preparation when a visible draft expires", () => {
    ...
    expect(screen.getByRole("button", { name: "Prepare as a portfolio" })).toBeInTheDocument();
    act(() => vi.advanceTimersByTime(1_100));
    expect(screen.queryByRole("button", { name: "Prepare as a portfolio" })).not.toBeInTheDocument();
...
describe("provider-neutral portfolio flow", () => {
  it("runs explicit safety verification once, keeps the portfolio visible, and hides unsupported actions", async () => {
    let release!: () => void;
    const onVerifyProvider = vi.fn(() => new Promise<void>((resolve) => { release = resolve; }));
    const routes: NonNullable<OrchestrationSnapshot["providerRoutes"]> = [
      { provider: "codex", label: "Codex", status: "blocked", verification: { state: "not_verified", canVerify: true } },
      { provider: "grok", label: "Grok Build", status: "blocked", verification: { state: "unsupported", canVerify: false } },
    ];
    render(<OrchestrateView {...props({ snapshot: { context, plans: [], runs: [], providerRoutes: routes, portfolioAssessments: [portfolioAssessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] }, onVerifyProvider })} />);
    fireEvent.click(screen.getAllByText("View worker readiness")[0]);
    const verify = screen.getByRole("button", { name: "Verify safety" });
    fireEvent.click(verify);
    fireEvent.click(verify);
    expect(onVerifyProvider).toHaveBeenCalledTimes(1);
    expect(screen.getByRole("article", { name: "Tonight's outcome plan" })).toBeVisible();
    expect(screen.getByText("Verifying…")).toBeDisabled();
    expect(screen.getAllByText("Verification unsupported")[0]).toBeVisible();
    expect(screen.queryByRole("button", { name: /Grok/ })).not.toBeInTheDocument();
    release();
    await waitFor(() => expect(screen.getByRole("button", { name: "Verify safety" })).toBeEnabled());
  });
  it("opens an outcome detail modal and hands that exact result to Morrow for revision", () => {
    const onDiscussPortfolio = vi.fn();
    render(<OrchestrateView {...props({
      snapshot: { context, plans: [], runs: [], providerRoutes: portfolioRoutes, portfolioAssessments: [portfolioAssessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] },
      onDiscussPortfolio,
    })} />);

    const plan = screen.getByRole("article", { name: "Tonight's outcome plan" });
    expect(plan).not.toHaveTextContent("npm test -- OrchestrateView");
    fireEvent.click(screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" }));
    const dialog = screen.getByRole("dialog", { name: "The five-step portfolio flow works" });
    expect(within(dialog).getByRole("button", { name: "Close outcome details" })).toHaveFocus();
    expect(dialog).toHaveTextContent("npm test -- OrchestrateView");
    expect(within(dialog).getByText("View exact execution scope").closest("details")).not.toHaveAttribute("open");
    fireEvent.click(within(dialog).getByRole("button", { name: "Revise this outcome with Morrow" }));
    expect(onDiscussPortfolio).toHaveBeenCalledWith(portfolioPlan, portfolioPlan.items[0]);
    expect(screen.queryByRole("dialog", { name: "The five-step portfolio flow works" })).not.toBeInTheDocument();
  });

  it("contains keyboard focus, locks the board, and restores the outcome trigger after closing", () => {
    render(<OrchestrateView {...props({
      snapshot: { context, plans: [], runs: [], providerRoutes: portfolioRoutes, portfolioAssessments: [portfolioAssessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] },
    })} />);
    const trigger = screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" });
    trigger.focus();
    fireEvent.click(trigger);
    const dialog = screen.getByRole("dialog", { name: "The five-step portfolio flow works" });
    const close = within(dialog).getByRole("button", { name: "Close outcome details" });
    const discuss = within(dialog).getByRole("button", { name: "Revise this outcome with Morrow" });
    const board = trigger.closest(".orchestrate-view") as HTMLElement;
    expect(dialog.closest(".orchestrate-view")).toBeNull();
    expect(close).toHaveFocus();
    expect(board.style.overflow).toBe("hidden");

    fireEvent.keyDown(window, { key: "Tab", shiftKey: true });
    expect(discuss).toHaveFocus();
    ...
    expect(board.style.overflow).toBe("");
  });

  it("closes outcome details and removes approval when the visible Night Plan expires", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-08-26T07:00:00.000Z"));
    const soonExpiredPlan = { ...portfolioPlan, expiresAt: "2026-08-26T07:00:01.000Z" };
    render(<OrchestrateView {...props({
      snapshot: {
        context,
        plans: [],
        runs: [],
        providerRoutes: portfolioRoutes,
        portfolioAssessments: [portfolioAssessment],
        portfolioPlans: [soonExpiredPlan],
        portfolioRuns: [],
      },
    })} />);

    fireEvent.click(screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" }));
    expect(screen.getByRole("dialog", { name: "The five-step portfolio flow works" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Approve once & start 2 results" })).toBeInTheDocument();

    act(() => vi.advanceTimersByTime(1_100));

    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /Approve once & start .* results/ })).not.toBeInTheDocument();
    expect(screen.getByRole("article", { name: "Edit portfolio to fit the night" })).toBeInTheDocument();
  });

  it("carries an unplanned candidate into Morrow so it can be added through conversation", () => {
    const onDiscussPortfolio = vi.fn();
    render(<OrchestrateView {...props({
      snapshot: { context, plans: [], runs: [], providerRoutes: portfolioRoutes, portfolioAssessments: [portfolioAssessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] },
      onDiscussPortfolio,
    ...
    fireEvent.click(screen.getByText("4 outcomes Morrow considered"));
    const candidate = screen.getByRole("heading", { name: "Choose the release scope", level: 3 }).closest("article")!;
    fireEvent.click(within(candidate).getByRole("button", { name: "Discuss this candidate with Morrow" }));

    expect(onDiscussPortfolio).toHaveBeenCalledWith(portfolioPlan, portfolioAssessment.candidates[2]);
  });
  ...
  it("shows each candidate's durable conversation evidence without exposing identifiers or raw text", () => {
    ...
    render(<OrchestrateView {...props({ snapshot: { context, plans: [], runs: [], providerRoutes: portfolioRoutes, portfolioAssessments: [assessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] } })} />);

    fireEvent.click(screen.getByText("4 outcomes Morrow considered"));
    const candidate = screen.getByRole("heading", { name: "Repair the portfolio UI", level: 3 }).closest("article")!;
    const disclosure = within(candidate).getByText("View 2 evidence conversations");
    ...
    expect(within(details).getByText("Codex")).toBeVisible();
    expect(within(details).getByText("Claude")).toBeVisible();
  });

  it("keeps an execution-blocked narrow candidate visible but outside the approvable plan", () => {
    const reason = "Claude Code cannot currently prove the exact approved execution and write boundary for this item. The scope was not widened and this item cannot be approved yet.";
    const assessment = {
      ...portfolioAssessment,
      candidates: portfolioAssessment.candidates.map((candidate) => candidate.stableKey === "write-copy" ? {
        ...candidate,
        disposition: "clarify" as const,
        rationale: `${candidate.rationale} ${reason}`,
        reasonCodes: [...candidate.reasonCodes, "executor_unavailable" as const],
        questions: [reason],
        writeScopes: ["README.md"],
      } : candidate),
    };
    const runnablePlan = {
      ...portfolioPlan,
      items: [portfolioPlan.items[0]],
      totalMinutes: portfolioPlan.items[0].estimatedMinutes,
      peakParallelism: 1,
    };
    render(<OrchestrateView {...props({ snapshot: {
      context,
      plans: [],
      runs: [],
      providerRoutes: portfolioRoutes,
      portfolioAssessments: [assessment],
      portfolioPlans: [runnablePlan],
      portfolioRuns: [],
    } })} />);

    expect(screen.getByRole("article", { name: "Tonight's outcome plan" })).toHaveTextContent("1 outcome by morning");
    expect(screen.queryByRole("checkbox", { name: "Include Polish the handoff copy" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByText("4 outcomes Morrow considered"));
    const blocked = screen.getByRole("heading", { name: "Polish the handoff copy", level: 3 }).closest("article")!;
    expect(blocked).toHaveTextContent("Needs your answer");
    expect(blocked).toHaveTextContent(reason);
    const disclosure = within(blocked).getByText("View 1 evidence conversation");
    fireEvent.click(disclosure);
    expect(within(blocked).getByText("Product copy")).toBeVisible();
  });
  ...
  it("keeps every disposition visible and completes edit, replan, separate approval, and run", async () => {
    ...
    expect(screen.getByRole("article", { name: "Tonight's outcome plan" })).toHaveTextContent("2 outcomes by morning");
    fireEvent.click(screen.getByText("4 outcomes Morrow considered"));
    ...
    fireEvent.click(screen.getByRole("button", { name: "Review 1 changed outcomes" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "Approve once & start 1 results" }));
  ...
  it("makes zero selected work an explicit no-approval state", () => {
    ...
    fireEvent.click(screen.getByRole("checkbox", { name: "Include Repair the portfolio UI" }));
    fireEvent.click(screen.getByRole("checkbox", { name: "Include Polish the handoff copy" }));
    ...
    expect(screen.getByRole("button", { name: "Review 0 changed outcomes" })).toBeDisabled();
  });
  ...
  it("offers only ready workers and preserves edits through a stale refresh", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" }));
    const dialog = screen.getByRole("dialog", { name: "The five-step portfolio flow works" });
    const select = within(dialog).getByRole("combobox", { name: /worker for this outcome/i });
    ...
    expect(screen.getByRole("checkbox", { name: "Include Polish the handoff copy" })).not.toBeChecked();
    expect(screen.getByRole("combobox", { name: /worker for this outcome/i })).toHaveValue("claude");
    expect(screen.getByRole("article", { name: "Tonight's outcome plan" })).toBeVisible();
  });

  it("keeps the user's outcome mix and worker choice when replanning fails", async () => {
    const onReplanPortfolio = vi.fn(async () => {
      throw new Error("Capacity changed before the revised plan could be scheduled.");
    });
    render(<OrchestrateView {...props({
      snapshot: { context, plans: [], runs: [], providerRoutes: portfolioRoutes, portfolioAssessments: [portfolioAssessment], portfolioPlans: [portfolioPlan], portfolioRuns: [] },
      onReplanPortfolio,
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" }));
    fireEvent.change(screen.getByRole("combobox", { name: /worker for this outcome/i }), { target: { value: "claude" } });
    fireEvent.click(screen.getByRole("button", { name: "Close outcome details" }));
    fireEvent.click(screen.getByRole("checkbox", { name: "Include Polish the handoff copy" }));
    fireEvent.click(screen.getByRole("button", { name: "Review 1 changed outcomes" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("Capacity changed before the revised plan could be scheduled.");
    expect(screen.getByRole("checkbox", { name: "Include Polish the handoff copy" })).not.toBeChecked();
    fireEvent.click(screen.getByRole("button", { name: "Open details for The five-step portfolio flow works" }));
    expect(screen.getByRole("combobox", { name: /worker for this outcome/i })).toHaveValue("claude");
    expect(screen.getByRole("button", { name: "Close outcome details" })).toHaveFocus();
  });
  ...
  it("keeps a ready plan runnable while exposing only the exact items from a separate selection draft", async () => {
    ...
    expect(screen.getByRole("button", { name: "Approve once & start 1 results" })).toBeEnabled();
    fireEvent.click(screen.getByText("5 outcomes Morrow considered"));
  ...
  it("shows itemized partial Morning Review with native receipts even when the plan is unavailable", () => {
    ...
    const review = screen.getByRole("article", { name: "Portfolio morning review" });
    expect(review).toHaveTextContent("Some reports need review");
    expect(within(review).getByRole("heading", { name: "2 overnight outcomes are ready" })).toBeVisible();
    expect(within(review).getByRole("heading", { name: "The flow works" })).toBeVisible();
    expect(within(review).getByRole("heading", { name: "Copy is clear" })).toBeVisible();
