...
`npm run dogfood:electron:real-codex` is an explicit live-provider smoke: it
uses the production worker and guard to let the locally authenticated Codex
write and verify one proof file in an isolated operating-system temporary
directory. It never uses the repository as the execution root and is not part
of CI or the normal `npm run check` gate.
